...
for file in *.md;  
	do name=${file%.*}
		...
		docker run --rm \
       	--volume "$(pwd):/data" \
       	--user $(id -u):$(id -g) \
       	pandoc/core:2.18 -s -f markdown -t $output_format --filter=pandoc-crossref --citeproc --csl $csl --reference-doc $templates_dir/$template $file -o $output_dir/$name-$output_name;
